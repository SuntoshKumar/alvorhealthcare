      ...
      <div className="container">
        <div className="grid items-center gap-12 lg:grid-cols-2 lg:gap-16">
            ...
            <div className="mb-8 grid gap-4 sm:grid-cols-2">
              {highlights.map((highlight) => (
                <div key={highlight.title} className="flex gap-3">
                  <div className="mt-0.5 flex h-9 w-9 flex-shrink-0 items-center justify-center rounded-lg bg-blue-50 dark:bg-blue-900/30">
                    <highlight.icon className="h-4.5 w-4.5 text-blue-600 dark:text-blue-400" />
                  </div>
                  <div>
          ...
          <div className="relative">
            <div className="relative aspect-[4/3] overflow-hidden rounded-3xl border border-blue-100 bg-gradient-to-br from-slate-50 via-blue-50 to-teal-50 p-6 dark:border-blue-800/30 dark:from-neutral-900 dark:via-blue-950/40 dark:to-teal-950/30 sm:p-8">
              <div className="pharma-grid absolute inset-0 opacity-35 dark:opacity-15" aria-hidden="true" />
              <div className="relative flex h-full flex-col justify-between">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-xs font-bold uppercase tracking-[0.18em] text-blue-600 dark:text-blue-400">Supply network</p>
                ...
                <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
                  {supplySteps.map((step, index) => (
                    <div key={step.label} className="relative rounded-2xl border border-white/80 bg-white/75 p-4 text-center shadow-sm backdrop-blur dark:border-white/10 dark:bg-neutral-900/70">
                      <step.icon className="mx-auto h-6 w-6 text-blue-600 dark:text-blue-400" />
                      <p className="mt-2 text-xs font-bold text-neutral-800 dark:text-neutral-100">{step.label}</p>
                      {index < supplySteps.length - 1 && (
                        <ArrowRight className="absolute -right-3 top-1/2 z-10 hidden h-4 w-4 -translate-y-1/2 text-blue-300 sm:block" aria-hidden="true" />
                      )}
                    </div>
                  ))}
                </div>
                ...
                <div className="flex flex-wrap gap-2">
                  {["Batch traceability", "Storage oversight", "Delivery coordination"].map((control) => (
                    <span key={control} className="rounded-full border border-blue-100 bg-white/70 px-3 py-1.5 text-xs font-semibold text-blue-700 dark:border-blue-800 dark:bg-blue-950/50 dark:text-blue-300">
                      {control}
                    </span>
                  ))}
                </div>
